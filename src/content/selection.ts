// Content script for automatic text selection fact-checking
import { SelectionContext, BackgroundMessage, FactCheckRequestMessage } from '../shared/types';
...
class SelectionHandler {
  ...
  private readonly DEBOUNCE_DELAY = 1000; // 1 second delay after selection
  private readonly MIN_TEXT_LENGTH = 20; // Minimum text length to trigger fact-check
  ...
  private init() {
    ...
    chrome.runtime.onMessage.addListener((message: BackgroundMessage, sender, sendResponse) => {
      if (message.type === 'FACT_CHECK_REQUEST') {
        this.handleFactCheckRequest(message as FactCheckRequestMessage, sendResponse);
  ...
  // Set up extension context invalidation detection
  private setupContextInvalidationDetection() {
  ...
  private autoFactCheck(selection: SelectionContext) {
  ...
  private handleFactCheckRequest(
  ...
  private getCurrentSelection(): SelectionContext | null {
  ...
  // Show selection indicator
  private showSelectionIndicator(text: string) {
  ...
  private showFactCheckResult(result: any) {
  ...
  private showResultOverlay(result: any) {
          ...
          ${result.sources.map((source: any, index: number) => `
            <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 12px; margin-bottom: 8px; background: #f9fafb;">
              ...
              <div style="font-size: 12px; color: #374151;">
                ${source.evidence.map((evidence: string) => `• ${evidence}`).join('<br>')}
              </div>
            </div>
          `).join('')}
        </div>
